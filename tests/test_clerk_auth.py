...
auth = ClerkAuth(app, secret_key="aStaticSecretKey!", log_signins=True,
                 auth_protect_layouts=True, page_container='_pages_content', public_routes=['/', '/user/<user_id>/public'])
...
def test_clerk_auth_flow(dash_duo):
    ...
    assert "clerk" in dash_duo.driver.page_source.lower() or "sign-in" in dash_duo.driver.current_url
    ...
    dash_duo.wait_for_text_to_equal("#_pages_content h1", "User john_doe (authenticated only)")
